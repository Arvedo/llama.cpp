...
void llama_model_zaya::load_arch_tensors(llama_model_loader &) {
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        if (i % 2 == 0) {
            ...
            layer.cca_val_proj1 = create_tensor(tn(LLM_TENSOR_CCA_VAL_PROJ1, "weight", i), { n_embd, n_embd_head }, 0);
            layer.cca_val_proj2 = create_tensor(tn(LLM_TENSOR_CCA_VAL_PROJ2, "weight", i), { n_embd, n_embd_head }, 0);
            ...
            layer.cca_conv_grp =
                create_tensor(tn(LLM_TENSOR_CCA_CONV_GRP, "weight", i), { d_conv, n_qk / n_groups, n_qk }, 0);
        ...
        layer.res_scale_res_b =
            create_tensor(tn(LLM_TENSOR_RES_SCALE_RES_B, "bias", i), { n_embd }, TENSOR_NOT_REQUIRED);

        // MoE layers (odd indices)
        if (i % 2 == 1) {
            // Router network
            layer.zaya_router_down =
                create_tensor(tn(LLM_TENSOR_ZAYA_ROUTER_DOWN, "weight", i), { n_embd, n_ff_exp }, 0);
            layer.zaya_router_down_b = create_tensor(tn(LLM_TENSOR_ZAYA_ROUTER_DOWN_B, "bias", i), { n_ff_exp }, 0);
            layer.zaya_router_norm   = create_tensor(tn(LLM_TENSOR_ZAYA_ROUTER_NORM, "weight", i), { n_ff_exp }, 0);
            layer.zaya_router_mlp0 =
                create_tensor(tn(LLM_TENSOR_ZAYA_ROUTER_MLP0, "weight", i), { n_ff_exp, n_ff_exp }, 0);
            layer.zaya_router_mlp0_b = create_tensor(tn(LLM_TENSOR_ZAYA_ROUTER_MLP0_B, "bias", i), { n_ff_exp }, 0);
            layer.zaya_router_mlp2 =
                create_tensor(tn(LLM_TENSOR_ZAYA_ROUTER_MLP2, "weight", i), { n_ff_exp, n_ff_exp }, 0);
            layer.zaya_router_mlp2_b = create_tensor(tn(LLM_TENSOR_ZAYA_ROUTER_MLP2_B, "bias", i), { n_ff_exp }, 0);
            layer.zaya_router_mlp4 =
                create_tensor(tn(LLM_TENSOR_ZAYA_ROUTER_MLP4, "weight", i), { n_ff_exp, n_expert + 1 }, 0);
            layer.zaya_router_biases =
                create_tensor(tn(LLM_TENSOR_ZAYA_ROUTER_BIASES, "weight", i), { n_expert + 1 }, TENSOR_NOT_REQUIRED);
            layer.zaya_router_eda_scale =
                create_tensor(tn(LLM_TENSOR_ZAYA_ROUTER_EDA_SCALE, "weight", i), { n_ff_exp }, TENSOR_NOT_REQUIRED);
...
llama_model_zaya::graph::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    const auto apply_res_scale = [&](ggml_tensor * x, ggml_tensor * scale, ggml_tensor * bias, const char * name,
                                     int il) {
        if (scale == nullptr) {
            return x;
        }
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        ggml_tensor * hidden_states =
            apply_res_scale(inpL, layer.res_scale_hs, layer.res_scale_hs_b, "res_scale_hs", il);
        if (residual != nullptr) {
        ...
        if (il % 2 == 0) {
            ...
            ggml_tensor * conv_state =
                ggml_view_3d(ctx0, cca_state, 2, n_qk, n_seqs, 2 * ggml_element_size(cca_state), cca_state->nb[1], 0);
            cb(conv_state, "cca_conv_state", il);

            ggml_tensor * prev_hs = ggml_view_2d(ctx0, cca_state, n_embd, n_seqs, cca_state->nb[1],
            ...
            if (n_seq_tokens > 1) {
                ggml_tensor * cur_shift =
                    ggml_view_3d(ctx0, cur_seq, n_embd, n_seq_tokens - 1, n_seqs, cur_seq->nb[1], cur_seq->nb[2], 0);
            ...
            ggml_tensor * last_conv_states = ggml_view_3d(ctx0, conv_input, 2, n_qk, n_seqs, conv_input->nb[1],
            ...
            ggml_tensor * conv_state_update_target =
                ggml_view_2d(ctx0, cca_state_all, conv_state_size, n_seqs, cca_state_all->nb[1],
            ...
            ggml_tensor * last_hs =
                ggml_view_2d(ctx0, cur_seq, n_embd, n_seqs, cur_seq->nb[2], (n_seq_tokens - 1) * cur_seq->nb[1]);
            ggml_tensor * prev_hs_update_target =
                ggml_view_2d(ctx0, cca_state_all, n_embd, n_seqs, cca_state_all->nb[1],
            ...
            for (int seq = 0; seq < n_seqs; ++seq) {
                ggml_tensor * conv_input_seq = ggml_view_2d(ctx0, conv_input, conv_input->ne[0], conv_input->ne[1],
                                                            conv_input->nb[1], seq * conv_input->nb[2]);

                ggml_tensor * QK_seq = ggml_conv_1d_dw(ctx0, conv_dw, conv_input_seq, 1, 0, 1);
                if (layer.cca_conv_dw_b) {
                    QK_seq = ggml_add(ctx0, QK_seq, ggml_reshape_3d(ctx0, layer.cca_conv_dw_b, 1, n_qk, 1));
                }

                QK_seq = ggml_conv_1d_grouped(ctx0, layer.cca_conv_grp, QK_seq, 1, 0, 1, n_groups);
                QK_seq = ggml_add(ctx0, QK_seq, ggml_reshape_3d(ctx0, layer.cca_conv_grp_b, 1, n_qk, 1));

                QK = QK == nullptr ? QK_seq : ggml_concat(ctx0, QK, QK_seq, 2);
            }
            cb(QK, "QK_dw", il);
            cb(QK, "QK_grp", il);
            ...
            ggml_tensor * K_conv =
                ggml_view_2d(ctx0, QK, n_embd_k, n_tokens, QK->nb[1], n_embd_q * ggml_element_size(QK));
            ...
            // GQA attention
            cur = build_attn(inp->get_attn(), layer.wo, nullptr, nullptr, Qcur, Kcur, Vcur, nullptr, nullptr, nullptr,
            ...
            ggml_tensor * gate_probs =
                ggml_cont(ctx0, ggml_view_2d(ctx0, router_probs, n_expert, n_tokens, router_probs->nb[1], 0));
    ...
    ggml_tensor * final_hidden =
        apply_res_scale(inpL, model.zaya_res_scale_hs, model.zaya_res_scale_hs_b, "final_res_scale_hs", -1);
    if (residual != nullptr) {
        residual =
            apply_res_scale(residual, model.zaya_res_scale_res, model.zaya_res_scale_res_b, "final_res_scale_res", -1);
